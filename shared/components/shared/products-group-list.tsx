...
import { useCategory } from '@/shared/store/category';
	...
	return (
		<div className={className} id={categoryId.toString()} ref={intersectionRef}>
			...
			<div className={cn('grid grid-cols-3 gap-[50px]', listClassName)}>
				{items.map((product, i) => (
					<ProductCard key={product.id} id={product.id} name={product.name} imageUrl={product.imageUrl} price={product.variation[0].price} />
				))}
			</div>
		</div>
	);
